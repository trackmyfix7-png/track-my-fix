...
export function WorkshopEntryPage() {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error || !data) {
          ...
          setWorkshop(data)
          // Armazena o slug para ser usado no callback pós-OAuth
          sessionStorage.setItem('pendingWorkshopSlug', slug)
          setState('ready')
        }
      })
  }, [slug])
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      // Supabase redireciona para /auth/callback, não volta aqui
    } catch {
      setState('ready')
    }
  }
    ...
    return (
      <div className="min-h-screen bg-brand-primary flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="mt-4 text-white/70">Link inválido ou oficina não encontrada.</p>
          <p className="mt-1 text-sm text-white/40">Entre em contato com sua oficina para obter o link correto.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-brand-primary flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center">
          ...
          <h1 className="text-2xl font-bold text-white">TrackMyFix</h1>
          <p className="mt-1 text-sm text-white/60">Acompanhe seu veículo em tempo real</p>
        </div>

        <Card className="shadow-xl border-0">
          <CardHeader className="pb-3">
            <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Oficina</p>
            <h2 className="text-xl font-bold text-brand-primary leading-tight">{workshop?.name}</h2>
            <p className="text-sm text-muted-foreground">Entre com sua conta Google para acessar o portal do cliente.</p>
          </CardHeader>
          <CardContent>
            <Button
              variant="outline"
