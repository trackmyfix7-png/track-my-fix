...
import { useOrderDetail, useUpdateOrderStatus } from '../hooks/useAdminVehicles'
  ...
  return (
    <div className="relative flex gap-4">
      ...
      <div className="pb-5 space-y-1">
        ...
        {notes && (
          <p className="mt-1 rounded-md bg-muted/60 px-3 py-2 text-sm text-foreground">
            {notes}
          </p>
        )}
      </div>
...
export function AdminVeiculoDetailPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`Placa ${v.plate} · OS #${orderId?.slice(0, 8).toUpperCase()}`}
        actions={
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        {/* Coluna esquerda — veículo + cliente */}
            ...
            <CardContent className="pt-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  ...
                  <span className={cn(
                    'font-semibold',
                    daysIn >= 4 ? 'text-brand-accent' : 'text-foreground'
                  )}>
              ...
              {order.exit_date && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium">
                    {format(parseISO(order.exit_date), 'dd/MM/yyyy', { locale: ptBR })}
                  </span>
                </div>
              )}
            </CardContent>
        ...
        {/* Coluna direita — histórico + checklist + atualizar */}
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={cn('font-medium text-right', mono && 'font-mono text-xs bg-muted px-2 py-0.5 rounded')}>
        {value}
      </span>
    </div>
  )
